...
def solve(input_data):
    ...
    for char_province in range(len(input_data)):
        if input_data[char_province]['name'].startswith('H'):
            provinces_startswith_h.append((input_data[char_province]['name'], input_data[char_province]['population']))

    for nums_province in range(len(input_data)):
        if input_data[nums_province]['population'] > 1000000:
            provinces_more_than_1m.append((input_data[nums_province]['name'], input_data[nums_province]['population']))

    #provinces_startswith_h = sorted(provinces_startswith_h, key=operator.itemgetter('name'))
    #provinces_more_than_1m = sorted(provinces_more_than_1m, key=operator.itemgetter('population'))
    provinces_startswith_h = sorted(provinces_startswith_h, key=operator.itemgetter(0))
    provinces_more_than_1m = sorted(provinces_more_than_1m, key=operator.itemgetter(1), reverse=True)
